...
- When AI exhausts autonomous testing (escalated status)
...
story = dict(conn.execute('''
    SELECT id, title, description, stage, status, notes
...
2. **Load verification report** from `${STORYTREE_DATA_DIR}/reviews/` or story notes
...
| Decision | New Stage | Status | Notes Update |
|----------|-----------|--------|--------------|
| Approved | `releasing` | `ready` | Append validation record |

...
from story_db_common import get_connection, get_next_child_id, generate_story_key
...
"""
    ...
    parent = conn.execute(
        'SELECT story_path, title FROM story_nodes WHERE story_path = ?', (args.parent_id,)
    ).fetchone()
    ...
    try:
        # Generate stable story key
        story_key = generate_story_key(conn)

        # Insert the new node
        conn.execute('''
            INSERT INTO story_nodes (story_path, title, description, stage, capacity, story_key)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (new_id, args.title, args.description, args.stage, args.capacity, story_key))

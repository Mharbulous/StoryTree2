...
def main():
    ...
    parent = conn.execute(
        'SELECT title FROM story_nodes WHERE story_path = ?', (new_parent_id,)
    ).fetchone()
    ...
    for story_id in story_ids:
        exists = conn.execute(
            'SELECT 1 FROM story_nodes WHERE story_path = ?', (story_id,)
        ).fetchone()
        if not exists:
    ...
    if dry_run:
        ...
        for story_id in story_ids:
            title = conn.execute(
                'SELECT title FROM story_nodes WHERE story_path = ?', (story_id,)
            ).fetchone()[0]

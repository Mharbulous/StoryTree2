...
Reports: statistics, stage/status/terminus distribution, structural issues.
...
Run these queries against `${STORYTREE_DATA_DIR}/story-tree.db` to surface semantic issues:
...
FROM story_nodes n
JOIN story_paths p ON p.descendant_id = n.id AND p.ancestor_id = 'root' AND p.depth = 1
WHERE n.terminus IN ('rejected', 'archived') OR n.status != 'ready';
```

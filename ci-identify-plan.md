...
3. Query the database at `${STORYTREE_DATA_DIR}/story-tree.db` to find matching stories:
...
- status: TEXT ('ready' if no blockers)
- terminus: TEXT (NULL if active in pipeline)
...
```bash
sqlite3 ${STORYTREE_DATA_DIR}/story-tree.db "SELECT id, title FROM story_nodes WHERE stage <> 'concept' AND terminus IS NULL"
```

...
If `${STORYTREE_DATA_DIR}/story-tree.db` doesn't exist, create directory and execute `references/schema.sql` via Python sqlite3, then insert root node and metadata.
...
- `status != 'ready'` (queued/pending/blocked/etc)
- `terminus IS NOT NULL` (rejected/archived/etc)
...
SELECT s.*,
    ...
    COALESCE(s.capacity, 3 + (SELECT COUNT(*) FROM story_paths sp
         ...
         AND child.terminus IS NULL)) as effective_capacity
FROM story_nodes s
...
### Key Rules
- **Cannot have both** non-ready status AND terminus (mutually exclusive)
- Stage is **preserved** when has non-ready status or disposed (know where to resume)
...
from story_db_common import (
    DB_PATH,                    # '${STORYTREE_DATA_DIR}/story-tree.db'
    MERGEABLE_STATUSES,         # {'concept', 'wishlisted', 'polish'} - concept=stage, others=status
    ...
    compute_effective_status,   # COALESCE(terminus, status, stage)
    delete_story,               # Cascade delete from story_nodes + story_paths
    reject_concept,             # Human rejection (terminus='rejected', goal signal)
    duplicative_concept,        # Algorithm duplicate (terminus='duplicative', no signal)
    conflict_concept,           # Inconsistent stories (status='conflicted', needs resolution)
    block_concept,              # Set status='blocked' with note
    defer_concept,              # Set status='escalated' with note
...
<!-- Subtree validation: 2026-01-06 - SyncoPaid->StoryTree push test -->

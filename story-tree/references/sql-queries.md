...
```bash
LAST_COMMIT=$(python -c "import sqlite3; c=sqlite3.connect('${STORYTREE_DATA_DIR}/story-tree.db'); print(c.execute(\"SELECT value FROM metadata WHERE key='lastAnalyzedCommit'\").fetchone()[0] or '')")
...
### Active Pipeline (not held, not terminal)
```sql
...
```sql
SELECT id, title, stage, status, notes FROM story_nodes
WHERE status != 'ready';
```
...
```sql
SELECT * FROM story_nodes
WHERE stage = 'approved' AND status = 'ready' AND terminus IS NULL;
```
...
SELECT * FROM story_nodes
WHERE stage = 'planned'
  AND status = 'ready' AND terminus IS NULL
ORDER BY updated_at ASC;
```

### Check Dependencies Before Implementation
```sql
-- Check if all children are at least planned (required before implementation)
...
WHERE stage = 'planned' AND status = 'blocked';
...
UPDATE story_nodes
SET status = 'ready', human_review = 0, stage = 'active',
...
```sql
UPDATE story_nodes
SET status = 'ready', human_review = 0
WHERE id = ?;
```

### Set Terminus (terminal)
```sql
UPDATE story_nodes
SET terminus = 'rejected', human_review = 0
WHERE id = ?;
```

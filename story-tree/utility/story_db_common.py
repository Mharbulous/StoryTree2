...
# Database path: .storytree/local/story-tree.db (relative to repo root)
# Navigate from: .claude/skills/storytree/story-tree/utility/ up to repo root
DB_PATH = str(Path(__file__).resolve().parent.parent.parent.parent.parent.parent / '.storytree' / 'data' / 'story-tree.db')

# Three-field workflow system status sets
# These represent computed status values from COALESCE(terminus, status, stage)

# Stages that don't participate in heartbeat processing
# epic = container-only nodes, shipped = completed work
NON_WORKFLOW_STAGES = {'epic', 'shipped'}

# Statuses that allow merging (early-stage concepts)
# Note: concept=stage, wishlisted/polish/refine=status
...
def generate_story_key(conn: sqlite3.Connection) -> str:
    """Generate a unique 6-char hex story key.

    Args:
        conn: SQLite connection (used to check for collisions)

    Returns:
        Unique 6-char hex string
    """
    import secrets

    while True:
        key = secrets.token_hex(3)  # 3 bytes = 6 hex chars
        existing = conn.execute(
            "SELECT 1 FROM story_nodes WHERE story_key = ?", (key,)
        ).fetchone()
        if not existing:
            return key


def get_story_by_key(conn: sqlite3.Connection, story_key: str) -> Optional[tuple]:
    """Look up a story by its stable key.

    Args:
        conn: SQLite connection
        story_key: 6-char hex key

    Returns:
        Tuple of (id, title, description) or None if not found
    """
    return conn.execute(
        "SELECT story_path, title, description FROM story_nodes WHERE story_key = ?",
        (story_key,)
    ).fetchone()


def get_story_key(conn: sqlite3.Connection, story_id: str) -> Optional[str]:
    """Get the story_key for a given story ID (path).

    Args:
        conn: SQLite connection
        story_id: Story ID (hierarchical path like "1.2.3")

    Returns:
        6-char hex story_key or None if story not found
    """
    row = conn.execute(
        "SELECT story_key FROM story_nodes WHERE story_path = ?", (story_id,)
    ).fetchone()
...
def get_story_version(conn: sqlite3.Connection, story_id: str) -> int:
    ...
    """
    row = conn.execute(
        'SELECT version FROM story_nodes WHERE story_path = ?', (story_id,)
    ).fetchone()
...
def compute_effective_status(
    ...
    Implements: COALESCE(terminus, status, stage)
    When status='ready', stage determines effective status.

    The three-field system uses:
    - stage: Position in pipeline (concept, planning, implementing, etc.)
    - status: Condition within stage (ready, queued, escalated, blocked, etc.)
    - terminus: Terminal state (rejected, archived, infeasible, etc.)

    Args:
        stage: Current pipeline stage
        status: Current status condition (default 'ready')
        terminus: Current terminus (if any)
...
def delete_story(conn: sqlite3.Connection, story_id: str) -> None:
    ...
    conn.execute('DELETE FROM story_nodes WHERE story_path = ?', (story_id,))
...
def reject_concept(
    ...
    """Set concept terminus to rejected (human decision, indicates non-goal).
...
def duplicative_concept(
    ...
    """Set concept terminus to duplicative (algorithm detected duplicate/overlap).
...
def block_concept(
    ...
    """Set concept status to blocked due to dependency on another story.

    This blocks a concept because it depends on another story being completed.
    The stage is preserved and status provides the effective state.
    ...
    conn.execute('''
        UPDATE story_nodes
        SET status = 'blocked', human_review = 1,
...
def defer_concept(
    ...
    """Set concept status to escalated for human review.
...
def conflict_concept(
    ...
    """Set concept status to conflicted (overlaps scope of another story in inconsistent way).
    ...
    conn.execute('''
        UPDATE story_nodes
        SET status = 'conflicted', human_review = 1,
...
def validate_tree_structure(conn: sqlite3.Connection) -> dict:
    ...
    all_nodes = conn.execute(
        'SELECT story_path, title FROM story_nodes WHERE story_path != ?', ('root',)
    ).fetchall()
...
def rename_story(
    ...
    for old, new in renames:
        ...
        # Update story_nodes
        conn.execute('UPDATE story_nodes SET story_path = ? WHERE story_path = ?', (temp_id, old))
    ...
    for old, new in renames:
        temp_id = temp_prefix + old
        # Update story_nodes
        conn.execute('UPDATE story_nodes SET story_path = ? WHERE story_path = ?', (new, temp_id))
        # Update story_paths
...
def rebuild_paths(conn: sqlite3.Connection, node_id: str) -> None:
    ...
    if parent_id:
        ...
        parent_exists = conn.execute(
            'SELECT 1 FROM story_nodes WHERE story_path = ?', (parent_id,)
        ).fetchone()
...
def rebuild_paths_recursive(conn: sqlite3.Connection, node_id: str) -> int:
    ...
    if node_id == 'root':
        ...
        descendants = conn.execute(
            "SELECT story_path FROM story_nodes WHERE story_path != 'root' ORDER BY LENGTH(story_path)"
        ).fetchall()
    else:
        # Find by ID prefix
        descendants = conn.execute('''
            SELECT story_path FROM story_nodes
            WHERE story_path = ? OR story_path LIKE ? ESCAPE '\\'
            ORDER BY LENGTH(story_path)
...
def move_story(
    ...
    story = conn.execute(
        'SELECT story_path, title FROM story_nodes WHERE story_path = ?', (story_id,)
    ).fetchone()
    if not story:
    ...
    parent = conn.execute(
        'SELECT story_path FROM story_nodes WHERE story_path = ?', (new_parent_id,)
    ).fetchone()
    if not parent:
    ...
    if new_parent_id == 'root':
        ...
        existing = conn.execute('''
            SELECT CAST(sn.story_path AS INTEGER) FROM story_nodes sn
            JOIN story_paths sp ON sn.story_path = sp.descendant_id
            WHERE sp.ancestor_id = 'root' AND sp.depth = 1
            AND sn.story_path GLOB '[0-9]*' AND sn.story_path NOT LIKE '%.%'
        ''').fetchall()
...
if __name__ == '__main__':
    ...
    print(f"  compute_effective_status('concept', 'ready', None) = '{compute_effective_status('concept', 'ready', None)}'")

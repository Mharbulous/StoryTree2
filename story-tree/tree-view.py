...
from storytree.config import get_db_path
...
COLORS = {
    ...
    'held': '\033[93m',         # Yellow (for any non-ready status)
    'terminal': '\033[90m',     # Gray (for any terminal state)
    'reset': '\033[0m'
}
...
class TreeNode:
    ...
    def __init__(self, id: str, title: str, stage: str, capacity: Optional[int],
                 ...
                 status: Optional[str] = None, terminus: Optional[str] = None):
        self.id = id
        self.title = title
        self.stage = stage
    ...
    def effective_status(self) -> str:
        ...
        if self.terminus:
            return 'terminal'
        elif self.status and self.status != 'ready':
            return 'held'
        else:
            return self.stage
...
def get_tree_data(db_path: Path, root_id: str = 'root', max_depth: Optional[int] = None,
    ...
    query = """
        SELECT
            s.story_path, s.title, s.description, s.stage, s.capacity,
            s.status, s.terminus,
            st.depth,
            ...
            (SELECT st2.ancestor_id FROM story_paths st2
             WHERE st2.descendant_id = s.story_path AND st2.depth = 1) as parent_id
        FROM story_nodes s

...
STATUS_SYMBOLS_UNICODE = {
    'epic': '📁',  # Container/folder - no workflow progression
...
ANSI_COLORS = {
    'epic': '\033[38;2;153;102;204m',  # Purple - container nodes
...
def find_default_db() -> Path:
    cwd = Path.cwd()
    # Primary location: .storytree/data/
    db_path = cwd / '.storytree' / 'data' / 'story-tree.db'
    if db_path.exists():
        return db_path
...
def get_tree_data(db_path: Path, root_id: str = 'root', max_depth: Optional[int] = None,
    ...
    query = """
        SELECT s.story_path, s.title, s.description,
            COALESCE(s.terminus, s.status, s.stage) AS status,
            s.capacity, st.depth,
            (SELECT COUNT(*) FROM story_paths WHERE ancestor_id = s.story_path AND depth = 1) as child_count,
            (SELECT st2.ancestor_id FROM story_paths st2 WHERE st2.descendant_id = s.story_path AND st2.depth = 1) as parent_id
        FROM story_nodes s
        JOIN story_paths st ON s.story_path = st.descendant_id
    ...
    if statuses:
        ...
        query += f" AND COALESCE(s.terminus, s.status, s.stage) {'NOT ' if exclude_statuses else ''}IN ({placeholders})"
        for i, status in enumerate(statuses):
            params[f'status_{i}'] = status

    query += " ORDER BY st.depth, s.story_path"
...
def build_tree(rows: list[dict]) -> tuple[Optional[TreeNode], list[TreeNode]]:
    ...
    nodes = {}
    for row in rows:
        nodes[row['story_path']] = TreeNode(
            id=row['story_path'], title=row['title'], status=row['status'],

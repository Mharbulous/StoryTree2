...
- When story stage is `verifying` with `status='broken'`
...
| Result | Transition |
|--------|------------|
| All failures fixed | Set `status='ready'`, keep stage at `verifying` for re-verification |
| Max attempts reached | Set `status='escalated'`, `human_review=1` |

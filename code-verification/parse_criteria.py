...
def parse_acceptance_criteria(description: str) -> list[dict]:
    ...
    - [ ] unchecked criterion
    - [x] or [X] checked criterion
    """
    criteria = []

    # Match checkbox format: - [ ] or - [x] or - [X]
    # Capture until next checkbox, double newline, or bold section
    pattern = r'-\s*\[([ xX])\]\s*(.+?)(?=\n-\s*\[|\n\n|\n\*\*|$)'
...
def get_story_criteria(story_id: str) -> dict:
    ...
    story = conn.execute('''
        SELECT story_path, title, description,
               COALESCE(terminus, CASE WHEN status = 'ready' THEN NULL ELSE status END, stage) AS display_state,
               project_path
        FROM story_nodes
        WHERE story_path = ?
    ''', (story_id,)).fetchone()
    ...
    criteria = parse_acceptance_criteria(story_dict.get('description') or '')

    return {
        "story_id": story_dict['story_path'],

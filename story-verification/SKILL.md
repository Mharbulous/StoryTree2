...
stories = [dict(row) for row in conn.execute('''
    ...
    WHERE s.stage IN ('verifying', 'reviewing')
      AND s.status = 'ready' AND s.disposition IS NULL
...
**Selection rules:**
- If user specified ID: validate exists and stage is `verifying` or `reviewing` (not has non-ready status/disposed)
...
| Any FAIL | Keep stage at `verifying`, set status='escalated', human_review=1 |
| All PASS but some UNTESTABLE | Keep stage at `verifying`, set status='escalated', human_review=1 |
| Mixed results | Interactive: ask user; CI: keep at `verifying` with non-ready status |
...
- **Three-Field System:** `.claude/skills/story-tree/SKILL.md` (stage + status + disposition)

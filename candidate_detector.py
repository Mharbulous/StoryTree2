...
# Supports both StoryTree (distributables/) and consumer (.claude/*/storytree/) structures
_script_dir = Path(__file__).parent.resolve()
# Try StoryTree structure: distributables/scripts/ → distributables/skills/story-tree/utility/
_utility_path = _script_dir.parent / 'skills' / 'story-tree' / 'utility'
if not _utility_path.exists():
    # Consumer structure: .claude/scripts/storytree/ → .claude/skills/storytree/story-tree/utility/
    _utility_path = _script_dir.parent.parent / 'skills' / 'storytree' / 'story-tree' / 'utility'
if _utility_path.exists():
    sys.path.insert(0, str(_utility_path))
else:
    raise ImportError(
        f"Cannot find story_db_common utility. Searched:\n"
        f"  {_script_dir.parent / 'skills' / 'story-tree' / 'utility'}\n"
        f"  {_script_dir.parent.parent / 'skills' / 'storytree' / 'story-tree' / 'utility'}"
    )
...
def load_stories(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    Computes status from three-field system: terminus > status (if not ready) > stage
    """
    conn.row_factory = sqlite3.Row
    return [dict(row) for row in conn.execute('''
        SELECT s.id, s.title, s.description,
               COALESCE(s.terminus, CASE WHEN s.status = 'ready' THEN NULL ELSE s.status END, s.stage) AS status
        FROM story_nodes s
        WHERE s.terminus IS NULL OR s.terminus NOT IN ('archived', 'deprecated')
        ORDER BY s.id
    ''').fetchall()]

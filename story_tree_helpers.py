...
from config import get_db_path, get_goals_dir

DB_PATH = str(get_db_path())
GOALS_DIR = str(get_goals_dir())
...
def get_prerequisites():
    ...
    if result['db_exists']:
        ...
        cursor = conn.cursor()
        # Three-field system: approved stage with ready status and no terminus
        cursor.execute("SELECT COUNT(*) FROM story_nodes WHERE stage = 'approved' AND status = 'ready' AND terminus IS NULL")
        result['approved_count'] = cursor.fetchone()[0]
        # Rejected terminus with notes
        cursor.execute("SELECT COUNT(*) FROM story_nodes WHERE terminus = 'rejected' AND notes IS NOT NULL AND notes != ''")
...
def get_rejected_stories():
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # Three-field system: rejected terminus with notes
    cursor.execute('''
        SELECT id, title, description, notes FROM story_nodes
        WHERE terminus = 'rejected' AND notes IS NOT NULL AND notes != ''
        ORDER BY id
    ''')
    for row in cursor.fetchall():
        print(f'=== {row[0]}: {row[1]} ===')

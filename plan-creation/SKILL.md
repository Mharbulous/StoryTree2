...
**Plans:** `${STORYTREE_DATA_DIR}/plans/`
...
stories = [dict(row) for row in conn.execute('''
    ...
    FROM story_nodes s
    WHERE s.stage = 'planning' AND s.status = 'ready' AND s.terminus IS NULL
...
- If user specified ID: validate exists and `stage = 'planning'` with `status = 'ready'` and no terminus
...
```bash
ls ${STORYTREE_DATA_DIR}/plans/*.md | tail -5
```
...
**For LOW complexity:** Single plan file → `${STORYTREE_DATA_DIR}/plans/NNN_[story-slug].md`

**For MEDIUM/HIGH complexity:** Multiple handover files → `${STORYTREE_DATA_DIR}/plans/NNN_[story-slug]-[component].md`
...
**Filename:** `${STORYTREE_DATA_DIR}/plans/NNN_[story-slug].md`
...
Plan complete and saved to `${STORYTREE_DATA_DIR}/plans/[filename]`.
...
**Output path:** `${STORYTREE_DATA_DIR}/plans/NNN_[story-slug]-[component].md`
...
conn.execute('''
    ...
    SET stage = 'implementing',
        notes = COALESCE(notes || chr(10), '') || 'Plan: ${STORYTREE_DATA_DIR}/plans/[FILENAME]',
...
conn.execute('''
    UPDATE story_nodes
    SET stage = 'implementing',
        notes = COALESCE(notes || chr(10), '') || 'Sub-plans: ${STORYTREE_DATA_DIR}/plans/[START]-[END]_[slug]*.md',
        updated_at = datetime('now')
    WHERE id = '[STORY_ID]'
''')
conn.commit()
...
**Option 2: Fresh session** - Open new Claude Code session, say "Implement plan: ${STORYTREE_DATA_DIR}/plans/[filename]"
...
1. Implement: ${STORYTREE_DATA_DIR}/plans/045_[slug]-db-schema.md
   Verify: [verification command]

2. Implement: ${STORYTREE_DATA_DIR}/plans/046_[slug]-core-logic.md
   Verify: [verification command]
...
✓ Planned story [STORY_ID]: [Title]
  ...
  Plan: ${STORYTREE_DATA_DIR}/plans/[filename].md
  Tasks: [N] TDD cycles
...
✓ Planned story [STORY_ID]: [Title]
  ...
  Sub-plans: [count] files
    - ${STORYTREE_DATA_DIR}/plans/[NNN]_[slug]-[component].md
    - ${STORYTREE_DATA_DIR}/plans/[NNN+1]_[slug]-[component].md
    - ...

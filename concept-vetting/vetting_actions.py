...
def defer_concept(concept_id, conflicting_id):
    """Set a concept to escalated status due to scope overlap. Stage preserved."""
    conn = sqlite3.connect(DB_PATH)
...
def cache_decision(id_a, id_b, classification, action):
    ...
    row_a = conn.execute("SELECT story_path FROM story_nodes WHERE story_path = ?", (id_a,)).fetchone()
    row_b = conn.execute("SELECT story_path FROM story_nodes WHERE story_path = ?", (id_b,)).fetchone()

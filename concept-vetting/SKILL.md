...
Effective status is computed as `COALESCE(terminus, status_if_not_ready, stage)`.
...
- `status IN ('broken', 'queued', 'escalated', 'blocked', 'conflicted')`
...
The vetting system computes an effective status field using:
```sql
COALESCE(terminus, CASE WHEN status = 'ready' THEN NULL ELSE status END, stage) AS effective_status
```

Priority: terminus (terminal) > status (condition) > stage (position)

This means:
- Story with `terminus='rejected'` shows effective_status='rejected' regardless of stage
- Story with `status='escalated'` shows effective_status='escalated' (stage preserved for resume)
- Story with `status='ready'` shows that stage value as effective_status
...
- **HUMAN_REVIEW -> DEFER_PENDING**: Set concept `status = 'refine'` with note listing conflicting story-node IDs (e.g., "Scope overlap detected with story-node IDs: 3.2.1, 4.1.1.3")
...
3. **Invalidation**: When a story's `title`, `description`, `stage`, `status`, or `terminus` changes, increment its `version`. All cached pairs involving that story become stale.
...
```
*Note: The `status` field shown is the effective status, computed as `COALESCE(terminus, CASE WHEN status = 'ready' THEN NULL ELSE status END, stage)` from the three-field system.*
...
```python
# Effective status = COALESCE(terminus, status_if_not_ready, stage)
...
| BLOCK_CONCEPT | `python ...vetting_actions.py block <concept_id> <blocking_id>` | Set status=blocked with note |
...
5 concepts set to status='refine' for scope overlap resolution.
```
...
- **Three-Field System:** `.claude/skills/story-tree/SKILL.md` (stage + status + terminus)

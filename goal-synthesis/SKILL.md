...
Generate `goals.md` and `non-goals.md` in `${STORYTREE_DATA_DIR}/goals/`.
...
- Read existing `${STORYTREE_DATA_DIR}/goals/goals.md` for context (if exists)
- Write `${STORYTREE_DATA_DIR}/goals/goals.md` with sections: What We're Building, Target User, Core Capabilities, Guiding Principles, Footer with timestamp

**Agent 2 (non-goals)** - Only if `needs_non_goals_update` is true:
- Query: `python .claude/scripts/story_tree_helpers.py rejected`
- Read existing `${STORYTREE_DATA_DIR}/goals/non-goals.md` for context (if exists)
- Write `${STORYTREE_DATA_DIR}/goals/non-goals.md` with sections: Explicit Exclusions (with rejection reasons), Anti-Patterns, YAGNI Items, Philosophical Boundaries, Footer with timestamp

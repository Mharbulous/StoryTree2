...
# Three-field system: exclude concept stage, stories with non-ready status, and disposed stories
cursor.execute('''
...
# Stories with 'polish' status (limit 2)
cursor.execute('''
...
cursor.execute('''
    UPDATE story_nodes
    SET description = ?, stage = 'concept', status = 'ready', updated_at = datetime('now')
    WHERE id = ?
...
Stage:       concept → planning → executing → reviewing → verifying → implemented → ready → released
                 ↑                    ↑            ↑
Status:       polish | wishlisted ───────────────────┘
                                  (clears status, returns to stage)

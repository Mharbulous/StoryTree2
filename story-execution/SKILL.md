...
```bash
python -c "import sqlite3; print(sqlite3.connect('${STORYTREE_DATA_DIR}/story-tree.db').execute('SELECT id FROM story_nodes LIMIT 1').fetchone())"
```
...
Scan `${STORYTREE_DATA_DIR}/plans/` for earliest sequence-numbered plan file:

```python
python -c "
import os, re, json
plans_dir = '${STORYTREE_DATA_DIR}/plans'
...
4. Archive plan file to `${STORYTREE_DATA_DIR}/plans/implemented/`

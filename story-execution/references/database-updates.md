...
SQL patterns for updating story status during implementation.
...
### Implementation Started (Step 2 → proceeding)
...
conn.execute('''
    UPDATE story_nodes
    SET stage = 'active',
        notes = COALESCE(notes || chr(10), '') || 'Implementation started: ' || datetime('now'),
...
### Implementation Paused (CI Mode - blocking issues)
...
# Stage stays 'active', status indicates why stopped
conn.execute('''
    UPDATE story_nodes
    SET status = 'paused', human_review = 1,
...
conn.execute('''
    ...
    SET stage = 'verifying', human_review = 0,
        notes = COALESCE(notes || chr(10), '') || 'Implementation complete, awaiting verification: ' || datetime('now'),
        updated_at = datetime('now')
    WHERE id = ?
''', (story_id,))

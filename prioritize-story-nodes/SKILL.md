...
**Priority Lists:** `${STORYTREE_DATA_DIR}/priority-lists/`
...
This script:
- Filters out blocked, non-ready status, and disposed stories
...
- Outputs a sorted priority list to `${STORYTREE_DATA_DIR}/priority-lists/`
...
4. **If story should be blocked**, set status:
...
**Filename:** `${STORYTREE_DATA_DIR}/plans/YYYY-MM-DD-[story-id]-[slug].md`
...
conn.execute('''
    UPDATE story_nodes
    SET stage = 'planned',
        notes = COALESCE(notes || '\n', '') || 'Plan: ${STORYTREE_DATA_DIR}/plans/PLAN_FILENAME',
        updated_at = datetime('now')
    WHERE id = 'STORY_ID'
''')
conn.commit()

...
def duplicative_concept(concept_id, duplicate_of_id):
    """Set a concept to duplicative terminus (algorithm detected duplicate/overlap)."""
    conn = sqlite3.connect(DB_PATH)
...
def block_concept(concept_id, conflicting_id):
    """Set a concept to blocked status with conflict note (stage preserved)."""
    conn = sqlite3.connect(DB_PATH)
...
def defer_concept(concept_id, conflicting_id):
    """Set a concept to refine status due to scope overlap. Stage preserved."""
    conn = sqlite3.connect(DB_PATH)
